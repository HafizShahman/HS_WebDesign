                    ...
                    <table border="0.5" bordercolor="white" align="center">
                        ...
                        <tr style="height:35px;background-color: rgba(49, 57, 73, 0.5);">
                            ...
                            <td width="100" value="<%=resultSet.getString("link")%>" name="lin">
                                <button id="btndw" onclick="btninsert()" name="btndol" value="<%=resultSet.getString("link")%>"><a href="<%=resultSet.getString("link")%>" name="ll">Download</a></button>
                            </td>
                        </tr>
                        <%
                                }
        ...
        <%
            try {
                ...
                if (request.getParameter("btndol") != null) //check register button click event not null
                {
                    ...
                    link = request.getParameter("btndol"); //txt_email

                    PreparedStatement pstmt = null; //create statement

                    pstmt = con.prepareStatement("insert into userhistory(Email, bookid, booktitle, link) "
                            + "select storeuser.Email, booklist.bookid, booklist.booktitle, booklist.link from storeuser, booklist "
                            + "where storeuser.Email='" + session.getAttribute("login") + "' and booklist.link='" + link + "'"); //sql insert query
